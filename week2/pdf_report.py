...
def make_region_chart(df, out_path):
    ...
    by_region = (
        df.groupby("region")["revenue"].sum().sort_values().round(2)
    )
...
def build_pdf(df, by_region, by_month, chart_path, out_path):
    """Assemble the PDF from flowables."""
    ...
    month_table.setStyle(_table_style_with_total(-1))   # no total row here
...
def _table_style_with_total(num_rows):
    """Common table styling; last row is a total row."""
    return TableStyle([
...
def _add_page_number(canvas, doc):
    ...
    text = f"Sales Report — Page {page_num}"
    canvas.saveState()
